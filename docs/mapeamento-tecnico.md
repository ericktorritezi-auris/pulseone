...
### 5.60 Relatório Executivo de Gestão (ex-"One Page Executiva") — resumo pro Gestor apresentar à Diretoria (v1.8.0, ajustado em v1.8.1, v1.8.2 e v1.8.3)
...
- Pontos de força/melhoria: **(v1.8.2, substitui a abordagem da v1.8.1)** 3 **itens curtos** cada — uma palavra ou expressão de poucas palavras (ex: "Disciplina", "Entregas consistentes"), nunca uma frase inteira. Vêm de `PulseAiAnalysis.strengthsItems`/`improvementItems`, dois campos novos (`String[]`) preenchidos pela PRÓPRIA IA (Anthropic), na mesma chamada que já gera a Análise Preditiva do relatório individual (seção 5.62 tem o detalhe do prompt) — o One Page/Relatório Executivo só LÊ o que já foi salvo, nunca chama IA de novo nem improvisa truncamento de texto. Renderizados como "chips" (etiquetas arredondadas), não bullets de frase — combina melhor com item curto do que lista vertical. Se o gestor ainda não gerou a Análise IA daquele relatório, aparece "Sem análise IA gerada para este colaborador neste ciclo" no lugar — não dispara geração automática (custaria uma chamada de IA por colaborador toda vez que alguém clicasse no botão).
