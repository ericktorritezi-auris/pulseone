...
### 5.55 Duas correções de visão cruzada por área para gestor de várias áreas e Admin

Depois da correção 5.54, o Erick trouxe mais dois casos do mesmo padrão ("assumir 1 área/1 ciclo só" num sistema que já suporta várias simultâneas):

**a) Admin não conseguia acompanhar avaliações de TODOS os colaboradores, de todas as áreas.** O Admin não participa do Pulse (não avalia, não é avaliado, não tem área) — isso está correto e não mudou. Mas ele pediu uma visão de **supervisão geral**: acompanhar o progresso de avaliação de todo mundo, em qualquer área, sem entrar como gestor de nenhuma. Antes disso não existia: `/pulse-team` (tela "Avaliação do Time") era `@Roles(GESTOR)` só, e mesmo se o Admin tivesse acesso, o cálculo de área (`managedAreaIds`) resolvia pra lista vazia pra ele (Admin não tem `areaId` nem `managedAreas`), então apareceria sempre vazio.
   - `PulseTeamController` passou a aceitar `ADMIN` também (`@Roles(GESTOR, ADMIN)`).
   - `getCurrentTeamProgress` e `computeProgress`: quando quem pede é `ADMIN`, não aplica filtro de área nenhum — vê **todos** os ciclos abertos (de qualquer área) e, dentro de cada um, **todos** os colaboradores ativos (não só os de uma área específica). Gestor e colaborador continuam com exatamente o mesmo comportamento de antes (código deles não foi tocado, só adicionado um branch novo pro Admin).
   - Sidebar: novo item "Avaliação Geral" no menu do Admin, apontando pra mesma tela (`/pulse/time`) — antes esse link nem existia pra esse perfil.

**b) Gestor de várias áreas via só o placar/feedback recebido da própria área, mesmo gerenciando outras.** No painel do Gestor (`/dashboard`, card "Como cada área te avaliou" + score/NPS por área), o cálculo usava `findFirst` pra achar "o ciclo finalizado/arquivado mais recente do sistema todo" — exatamente o mesmo padrão de bug da 5.54, só que aqui dentro do dashboard, que não tinha sido varrido naquela rodada porque na época o filtro por `status: 'ABERTO'` (ciclo em andamento) foi o critério de busca, e esse ponto usa `FINALIZADO`/`ARQUIVADO` (ciclo já fechado). Como cada área tem seu próprio ciclo (com seu próprio `id`), esse único "mais recente" quase sempre era o ciclo de uma área só — as outras áreas que esse gestor também gerencia ficavam mostrando score vazio (`—`) e sumiam do card de avaliação recebida, mesmo ele sendo o gestor legítimo delas.
   - `DashboardService.getManagerDashboard`: em vez de um `latestCycle` único, agora busca o ciclo `FINALIZADO`/`ARQUIVADO` mais recente **de cada área gerida** (`Geral` ou da própria área), guardado num mapa por `areaId`. O score/NPS médio por área (`porArea`) e o card "como cada área te avaliou" (`avaliacaoRecebidaPorArea`) usam, cada um, o ciclo da **sua própria** área — não mais um só ciclo global misturando tudo.
   - Sem mudança nenhuma na tela (mesmo formato de resposta) nem no cálculo do score oficial do ciclo (que continua sendo um número único, como já documentado na 5.54/dashboard) — só a fonte de dados por área foi corrigida.
   - Comentário desatualizado removido: o código antigo dizia "gestor nunca gerencia mais de uma área no modelo atual", o que não é mais verdade desde a introdução de `managedAreas` (seção 5.25) — corrigido pra refletir o modelo atual.
...
### 5.59 Ciclos Pulse e Comunicados multi-área — resolve a fragmentação de score do gestor de várias áreas (v1.7.0)

**Origem:** discussão iniciada quando o Erick reparou que o próprio score dele (gestor de várias áreas) mostrava "Gestor (40%)" contando só os liderados da SUA área principal — os liderados das outras áreas que ele também gerencia (seção 5.54/5.55) simplesmente não entravam, porque `PulseScoreService.computeForCycle` soma feedback **estritamente dentro do mesmo `cycleId`**, e cada área historicamente abre seu PRÓPRIO ciclo (seu próprio `cycleId`). O Erick decidiu explicitamente **não mexer** nos ciclos já fechados ("Nada muda... Só queria essa explicação mesmo"), mas propôs a solução definitiva pra ciclos NOVOS: em vez de abrir um ciclo pra "uma área só ou todas (Geral)", permitir **marcar várias áreas específicas de uma vez** (checkbox) pra abrirem juntas, compartilhando o mesmo `cycleId` — e pediu o mesmo pra Comunicados.

**Por que isso resolve o problema sem tocar no motor de cálculo:** o bug nunca foi na fórmula do score (`teamScore*0.6 + managerScore*0.4`, seção 5.1) — foi na fragmentação dos dados em `cycleId`s diferentes. Se as áreas que um gestor gerencia passam a compartilhar o mesmo ciclo, todo o `AVALIACAO_GESTOR` recebido dos liderados de TODAS elas cai automaticamente no mesmo balde de `managerScore`, sem precisar mudar uma linha do `PulseScoreService`.

**Schema (aditivo, seguro pro `prisma db push` que roda a cada deploy — ver `start.js`):** `PulseCycle` e `Announcement` ganharam um novo relacionamento N:N `areas Area[]` (tabelas de junção novas, geradas automaticamente pelo Prisma), ao lado do `areaId String?` já existente. Nenhum ciclo/comunicado antigo é afetado — todos continuam com esse novo relacionamento vazio e resolvem normalmente pelo `areaId` legado. Utilitário novo e compartilhado, `backend/src/common/cycle-area.util.ts`:
- `resolveGroupAreaIds(entity)` — retorna a lista de áreas do grupo (do relacionamento `areas` se tiver 1+, senão do `areaId` único, senão `null` = GERAL/todas), usado em `PulseAssignmentService.generateForCycle` e em `pulse-team`/`dashboard`.
- `areaGroupWhere(areaIds)` — cláusula Prisma reaproveitada em toda consulta que hoje testava só `areaId`, agora também casando um ciclo/comunicado multi-área que inclui qualquer uma das áreas do array (`dashboard.module.ts`, `pulse-team.module.ts`, `announcements.module.ts`).
- `areaGroupLabel(entity)` — rótulo amigável ("Ipatinga + Chapecó") pras telas de listagem.

**Regra de gravação na criação (`PulseCyclesService.create` / `AnnouncementsService.create`):** 0 área marcada = GERAL (igual sempre); exatamente 1 área grava tanto no `areaId` legado quanto no novo relacionamento `areas` (compatibilidade total — nenhum código que só lê `areaId` precisa mudar pra esse caso, que é o mais comum hoje); 2+ áreas usa só o relacionamento `areas`, com `areaId` ficando `null`.

**`PulseAssignmentService.generateForCycle` — o que muda e o que NÃO muda:** a resolução de quais áreas processar passou a usar `resolveGroupAreaIds(cycle)` em vez de só `cycle.areaId`. O resto da função é idêntico: ela já processava área por área dentro de um `for`, então **Avaliação de Colegas continua estritamente dentro da mesma área** mesmo quando várias áreas compartilham o mesmo ciclo — dois colegas de áreas diferentes dentro do mesmo grupo multi-área nunca formam par. Já a Avaliação de Gestor (`AVALIACAO_EQUIPE`/`AVALIACAO_GESTOR`) já usava uma busca GLOBAL de usuários (seção 5.1) pra resolver o gestor mesmo fora da área — isso não muda, e é exatamente esse comportamento que agora fica **corretamente somado num só `cycleId`** quando as áreas do gestor abrem juntas. Os dois invariantes confirmados com o Erick permanecem intactos: colega nunca cruza área (Ipatinga não avalia Chapecó), e o gestor sempre avalia/é avaliado por liderados de TODAS as áreas que gerencia, independente de localização.

**Frontend:** em "Novo Ciclo Pulse" e "Novo Comunicado", a opção "Só uma área" virou "Áreas selecionadas" — uma lista de checkboxes (em vez de um único `<select>`) dentro de um card com rolagem, permitindo marcar quantas áreas quiser. As telas de listagem (Ciclos Pulse, Comunicados, e o já existente "Avaliação do Time"/"Avaliação Geral" do gestor/admin) passaram a mostrar o rótulo combinado ("Área A + Área B") quando o registro é multi-área, em vez de só a área única ou "Geral".

**Ciclos já fechados/antigos:** confirmado — comportamento zero alterado. Eles não têm nada no novo relacionamento `areas`, então toda a resolução cai no `areaId` de sempre, exatamente como funcionava antes desta mudança. O ganho vale só pra ciclos e comunicados abertos/criados a partir de agora, marcando várias áreas juntas.
...
### 5.64 CORREÇÃO DE PRIVACIDADE — "Meu Perfil → Dados Completos" vazava nome de quem deu feedback de colega no Pulse (v1.8.4)

**Como foi encontrado:** o Erick reportou, com print de tela, que ao entrar em "Meu Perfil" e clicar em "Dados Completos" (o Dossiê pessoal), a seção "Feedbacks recebidos (Pulse Setembro/2026)" mostrava o NOME REAL de cada colega que deu feedback naquele ciclo — quebrando o anonimato que o resto do sistema garante pra Avaliação de Colegas.

**Causa raiz:** o sistema já tem uma regra de anonimato bem estabelecida (PRD seção 19, aplicada em `pulse-reports.module.ts`/`buildReportDetail`, e também corrigida uma vez antes pro fluxo de PDF do relatório — seção 5.58): quando a PRÓPRIA pessoa está vendo seus dados, feedback de colega vira "Colega 1/2/3" e feedback de liderado (quando a pessoa é gestora) vira "Liderado 1/2/3" — só o gestor direto aparece com nome real. Quando é gestor/admin vendo os dados de OUTRA pessoa, todo mundo aparece com nome real (precisam pra gerir de verdade).

O módulo do Dossiê (`dossie.module.ts`) tem duas portas de entrada pra essencialmente a mesma função interna (`assembleDossie`): `getDossie(id, requester)` — usada pela tela de gestão (`/dossie/:id`, só ADMIN/GESTOR, vendo o dossiê de outra pessoa) — e `getMyDossie(requesterId)` — usada por "Meu Perfil → Dados Completos" (`/meu-dossie`, qualquer perfil vendo os PRÓPRIOS dados). A função interna `assembleDossie` **nunca recebia essa distinção** — não sabia se estava sendo chamada pela via de gestão ou pela via "eu vendo meus próprios dados" — e sempre montava os feedbacks do Pulse com o nome real do avaliador, com um comentário no código dizendo (erroneamente) que essa função "é uso interno de RH/gestão", sem perceber que a mesma função também alimentava a visão da própria pessoa.

**Correção:**
- `assembleDossie` ganhou um parâmetro `viewingAsOwner: boolean`.
- `getDossie()` (gestão vendo outra pessoa) passa `false` — comportamento inalterado, nome real continua aparecendo (é o uso pretendido, igual sempre foi pro relatório individual).
- `getMyDossie()` (a própria pessoa vendo seus dados) passa `true` — ativa a MESMA lógica de anonimato já usada em `pulse-reports.module.ts`: `AUTOAVALIACAO` → "Autoavaliação"; `AVALIACAO_EQUIPE` (gestor avaliando a pessoa) → sempre nome real; `AVALIACAO_GESTOR` (liderado avaliando a pessoa, quando ela é gestora) → "Liderado N"; `COLEGA` → "Colega N".
- O PDF do Dossiê (`GET /dossie/:id/pdf`) não precisou de nenhuma mudança — ele só existe na via de gestão (não existe rota de PDF pra "Meu Dossiê"), então já estava correto.
- **O que NÃO foi tocado, de propósito:** "Últimos Feedbacks Recebidos (Avulsos)" — o Feedback Contínuo (`model Feedback`, sem relação com o ciclo Pulse) é uma funcionalidade completamente diferente e sempre foi projetada como IDENTIFICADA (a pessoa que manda um feedback avulso assina com o próprio nome, tipo um reconhecimento direto) — mostrar o nome de quem mandou ali é o comportamento correto, não um vazamento.

**Varredura feita pra confirmar que não havia mais nenhum vazamento parecido:** busca por todo uso de `evaluator.fullName` no backend — só existiam os dois pontos citados acima (`pulse-reports.module.ts`, já correto, e `dossie.module.ts`, agora corrigido); e por todo `.autor`/nome de avaliador renderizado no frontend — Dashboard e Relatório individual já recebem os dados já anonimizados do backend (nada a corrigir ali), e o Dossiê era o único ponto quebrado.

**Nada muda pro uso normal:** quem é gestor/admin olhando o dossiê de outra pessoa continua vendo nome real de todo mundo (precisa pra gerir). O que muda é só quando a própria pessoa entra no PRÓPRIO perfil — a partir daqui, colega e liderado saem anonimizados, igual já acontecia em Relatórios e no Dashboard.

### 5.65 Cascata de conteúdo + multi-seleção de ciclos no Relatório Executivo — v1.8.5

**Motivação, relatada pelo Erick por voz (20/09):** na prática, depois da v1.8.3 entrar em produção, o Relatório Executivo (seção 5.60) frequentemente saía **vazio** — a maioria dos relatórios individuais nunca tinha recebido um clique em "Gerar Análise IA" antes do ciclo fechar (o gestor consolida o ciclo escrevendo o parecer, mas nem sempre passa pela Análise IA), então `strengthsItems`/`improvementItems` ficavam como array vazio pra quase todo mundo — "inapresentável pra Diretoria" nas palavras dele. Ao mesmo tempo, ele esclareceu que abre o Pulse **separado por área** (um ciclo por área, não um ciclo único cobrindo todas), e o seletor de um ciclo só (v1.8.3) não dava pra "ter o valor real de todas as áreas" de uma vez.

**Processo seguido, por exigência explícita do Erick:** *"Antes de fazer qualquer coisa, analise esse meu pedido... Gera pra mim uma demo, mas uma demo real, de como isso funcionaria na prática... E aí, uma vez aprovado, você faz esse ajuste."* — foi construída uma demo funcional (dados fictícios, mas a mesma engine HTML/Puppeteer real), renderizada em PDF de verdade e enviada pra revisão antes de qualquer alteração no código de produção. Só depois de aprovada (com a ressalva adicional de que o critério do RH precisa continuar "bem criterioso" — ver abaixo) o código real foi alterado.

**1) Cascata de conteúdo por colaborador (`OnePageColaborador.fonte`, `pulse-report-pdf.service.ts` + `OnePageExecutivaService.build`, `pulse-reports.module.ts`):** cada colaborador agora cai em UMA de 4 fontes, na ordem de prioridade pedida pelo Erick — nunca mistura, nunca deixa o card sem explicação:
1. **`'items'`** — Análise IA já com os itens curtos (`strengthsItems`/`improvementItems`, seção 5.62) preenchidos. Melhor caso, renderizado como chips, igual antes.
2. **`'analise_textual'`** — a Análise IA existe, mas é de antes da v1.8.2 (só tem os campos de texto livre `strengths`/`improvements`/`trends`). Renderizado como 3 resumos em prosa: "Pontos fortes —", "A desenvolver —", "Tendência —" (cortados com segurança por palavra em até 150 caracteres cada, nunca no meio de uma palavra).
3. **`'parecer'`** — nunca foi gerada Análise IA nenhuma pra essa pessoa nesse relatório; usa o `PulseReport.managerFinalOpinion` (o parecer final que o próprio gestor escreveu pra fechar o relatório dela) como resumo (até 220 caracteres, mesmo corte seguro).
4. **`'nenhum'`** — nada registrado (nem Análise IA, nem parecer). Mostra a frase honesta "Sem parecer ou análise registrada para este colaborador neste ciclo." em vez de um card em branco sem explicação.

A resolução é puramente de LEITURA — não dispara nenhuma chamada de IA nova (mesma decisão de custo já tomada na seção 5.62: o Relatório Executivo nunca gera IA na hora de exportar).

**2) Multi-seleção de ciclos (substituindo o seletor único da v1.8.3):**
- `OnePageExecutivaService.build(requesterId, cycleIds?: string[])` — segundo parâmetro passou de `cycleId?: string` pra uma LISTA. Pra cada área gerida, usa o ciclo **mais recente dentre os marcados que efetivamente cobre aquela área** (checagem individual por área, na ordem de `openedAt desc` dos ciclos selecionados) — resolução de sobreposição confirmada explicitamente com o Erick via pergunta objetiva: nunca soma nem faz média entre ciclos que cobrem a mesma área, sempre usa um só, o mais novo. Sem `cycleIds` (chamada antiga/API direta), cai no fallback de sempre (último ciclo fechado que toca qualquer área gerida) — mantém compatibilidade.
- `GET /pulse-reports/one-page/pdf` aceita `?cycleIds=id1,id2,id3` (lista separada por vírgula); o parâmetro singular antigo (`?cycleId=...`) continua aceito por retrocompatibilidade (útil caso alguém tenha a tela antiga em cache no navegador no momento do deploy).
- **Tela (`/relatorios`):** o modal de seleção virou **multi-seleção por checkbox** (era single-select/radio) — nada vem pré-marcado por padrão (decisão confirmada explicitamente com o Erick: "Nenhum marcado, você escolhe manualmente" — não o "marca tudo automaticamente"). O botão "Gerar PDF" mostra a contagem de ciclos marcados e fica desabilitado com zero selecionados.
- **Capa do PDF:** o antigo texto único `cicloResumo` ("Ciclo: X") foi substituído por `ciclosUsados` — uma caixa "Ciclos usados neste relatório" listando cada ciclo efetivamente utilizado e quais áreas ele supriu (só entram ciclos que realmente supriram alguma área — um ciclo marcado que não cobre nenhuma área gerida simplesmente não aparece na lista). O indicador no canto superior também passou a dizer "N ciclos combinados" (ou "1 ciclo"). Cada página de área ganhou um subtítulo "Ciclo: <label>" (antes só a capa mostrava qual ciclo estava em uso).

**3) Reforço do critério de RH (confirmação do Erick após ver a demo: "nos pontos de RH só tem que aparecer aquilo que realmente fizer sentido... uma análise bem criteriosa mesmo"):** a lógica de sinalização em si **não mudou** — já era criteriosa antes desta versão (score < 60 OU queda ≥10 pontos pra "Plano de desenvolvimento"; tenure > 12 meses **e** score ≥ 75 pra "Valorização" — seção 5.60/5.61) e continua exatamente assim. O que mudou foi só deixar isso mais explícito no próprio PDF: a página de RH agora informa o número exato de colaboradores que NÃO aparecem ali por não baterem nenhum critério (ex: "4 colaborador(es) sem nenhum ponto de atenção não aparecem nesta página"), reforçando visualmente pra quem lê que a ausência de alguém ali é intencional, não uma omissão.

**Verificação feita antes da entrega:** a função real `PulseReportPdfService.buildOnePageHtml()` foi compilada isoladamente (TypeScript real, sem alterar a lógica) e executada com dados simulados reproduzindo o cenário do Erick (3 áreas, 2 ciclos combinados, um colaborador em cada uma das 4 fontes, RH mostrando só 2 de 6 pessoas) — o HTML gerado foi renderizado em PDF de verdade via Puppeteer/Chromium (mesma engine de produção) e revisado página a página antes deste pacote ser fechado.

### 5.60 Relatório Executivo de Gestão (ex-"One Page Executiva") — resumo pro Gestor apresentar à Diretoria (v1.8.0, ajustado em v1.8.1, v1.8.2, v1.8.3 e v1.8.5)

**Origem:** pedido do Erick pra dar ao Gestor uma forma de resumir a atuação dele em TODAS as áreas que gerencia — pronto pra levar à Diretoria: área, colaboradores, score de cada um, pontos fortes/melhoria, e o score geral do gestor (média das áreas). Antes de construir de verdade, foi gerado um PDF de demonstração com dados fictícios pra validar o layout — aprovado o formato, o Erick esclareceu um ponto sobre o cruzamento salarial (ver abaixo) e autorizou a implementação real. Nasceu como "One Page" (uma página só, A4 paisagem); virou um **relatório executivo de várias páginas** na v1.8.2 (ver "Layout e paginação" abaixo) — o endpoint e o nome interno (`OnePageExecutivaService`, `one-page/pdf`) ficaram como estavam por não afetar nada visível, só o rótulo do botão e o texto do PDF mudaram.

**Botão:** "Gerar Relatório Executivo" (rótulo da v1.8.2 — era "Gerar One Page Executiva") no topo da tela de Relatórios (`/relatorios`), visível só pra GESTOR — decisão explícita de manter um botão único que gera a visão de TODAS as áreas geridas de uma vez, em vez de um botão por área/ciclo.
...
- Pontos de força/melhoria: **(v1.8.2, substitui a abordagem da v1.8.1; ver v1.8.5/seção 5.65 pra cascata completa)** quando a Análise IA já tem os itens curtos gerados, são 3 **itens curtos** cada — uma palavra ou expressão de poucas palavras (ex: "Disciplina", "Entregas consistentes"), nunca uma frase inteira. Vêm de `PulseAiAnalysis.strengthsItems`/`improvementItems`, dois campos (`String[]`) preenchidos pela PRÓPRIA IA (Anthropic), na mesma chamada que já gera a Análise Preditiva do relatório individual (seção 5.62 tem o detalhe do prompt) — o Relatório Executivo só LÊ o que já foi salvo, nunca chama IA de novo nem improvisa truncamento de texto. Renderizados como "chips" (etiquetas arredondadas), não bullets de frase. **Superado pela v1.8.5:** sem os itens curtos, o relatório não fica mais vazio — cai numa cascata de 3 outros fallbacks (texto livre da Análise IA antiga → parecer final do gestor → nota honesta de "nada registrado"), documentada por completo na seção 5.65.
  - **Histórico:** na v1.8.0 era 1 frase (cortava). Na v1.8.1, virou até 3 frases fatiadas do texto livre existente com corte seguro por palavra (`topSentences()`/`truncateAtWord()`) — melhor, mas ainda eram frases longas, não itens. O Erick corrigiu explicitamente: "não é isso... eu quero que a gente condense num bullet com três pontos... pode ser uma frase, pode ser uma composição, mas [não] frase inteira" — daí a v1.8.2 mover a síntese pra dentro do próprio prompt de IA (que já enxerga os comentários todos) em vez de tentar derivar itens curtos de um texto livre já pronto. `topSentences()`/`truncateAtWord()` foram removidos do código (não são mais usados em lugar nenhum).
...
**Faixa de Pontos de Atenção para o RH — a parte pedida explicitamente pelo Erick ("valorização de funcionários... tempo de casa e média salarial"):** cruza score, tempo de casa e salário (Dossiê Confidencial) e aplica regras, em ordem de prioridade (cada pessoa recebe no máximo UM sinalizador, o mais relevante):
1. **Plano de desenvolvimento** — score atual abaixo de 60, OU queda de 10+ pontos em relação ao ciclo anterior do mesmo colaborador. Sem gate de tempo de casa (um desempenho ruim precisa de atenção independente de quanto tempo a pessoa está na empresa).
2. **Valorização** — **(v1.8.1, unifica as antigas "Risco de retenção" e "Reconhecimento pendente" da v1.8.0 num único sinalizador, por pedido do Erick)**. Critério de entrada: **mais de 1 ano de casa** (`tenureMonths > 12` — regra explícita: "sempre considerar nessa avaliação funcionários com mais de 1 ano de casa") **e** score bom neste ciclo (≥75). Quem entra, mostra **3 pontos de destaque primeiro** — o motivo da valorização vem antes de qualquer número, exatamente como pedido: "Precisamos colocar 3 pontos do porque o RH deveria valorizar e aí sim trazer o quanto o salário do colaborador está defasado". **(v1.8.2)** Esses 3 pontos deixaram de ser linhas fixas montadas no código (score/banda + tempo de casa + "topStrength") e passaram a vir de `PulseAiAnalysis.valuationItems` — a mesma IA que gera a Análise Preditiva sintetiza 3 motivos curtos de valorização (ex: "Referência técnica na área", "Baixo turnover de conhecimento"), com uma instrução explícita no prompt pra NÃO falar de salário nesses 3 itens (o salário é tratado à parte, como consequência, não motivo). Sem Análise IA gerada ainda pro relatório da pessoa, cai num fallback textual simples (score/banda + tempo de casa) só pra não deixar o card vazio. **Só depois** desses 3 pontos aparece a linha de salário, separada visualmente por um traço: percentual de defasagem em relação à média salarial do MESMO CARGO (por nome, não por `positionId`) **entre todas as áreas geridas pelo gestor** — não a média da empresa toda, nem só da própria área (correção explícita do Erick: "comparado com todos os cargos de mesmo nível e intensidade dentro de TODAS AS ÁREAS DE ATUAÇÃO DO GESTOR"); só calcula defasagem com pelo menos 2 pessoas do mesmo cargo com salário cadastrado (senão não existe "média" de verdade) — quando não há dados suficientes, ou quando o salário já está alinhado/acima da média, a linha final é ajustada nesse sentido em vez de mostrar um percentual negativo sem sentido. Cargo é comparado pelo NOME (`position.name`, normalizado) e não pelo `positionId` pelo mesmo motivo já documentado: cargo é escopado por área (`@@unique([name, areaId])`), então "Analista" em Ipatinga e em Chapecó são registros `Position` diferentes.

Limitado a 6 alertas na página de RH (prioriza desenvolvimento > valorização), pra não deixar a página excessivamente longa.

**Acesso ao salário:** confirmado com o Erick — só o próprio gestor que está gerando o relatório vê o valor (nunca o número exato do salário aparece no PDF, só o percentual de defasagem e o selo "Valorização"), mesmo escopo de acesso que o Dossiê Confidencial já tem hoje (visível só pra ADMIN/GESTOR, nunca pro colaborador).

**PDF:** A4 PAISAGEM (o relatório individual continua RETRATO) — `PulseReportPdfService.generatePdf()` tem um terceiro parâmetro opcional `{ landscape }`, default `false`, então a chamada do relatório individual não muda em nada. Método `buildOnePageHtml()` no mesmo serviço, reaproveitando a mesma engine Puppeteer/Chromium já configurada pra produção (seção do `railpack.json`/`aptPackages: chromium`).

**Layout e paginação (reformulado na v1.8.2, por pedido explícito do Erick):** deixou de ser uma página única espremida — virou um **relatório executivo de várias páginas**, cada uma com cabeçalho próprio e conteúdo espaçado ("bonitinho", nas palavras do Erick — "eu não quero que fique de uma página, porque quero que separe direitinho"). Estrutura:
1. **Capa** — cabeçalho completo (marca, ciclo, data, selo confidencial), faixa do gestor, os 4 indicadores gerais (score geral, colaboradores, áreas em zona verde, pontos de atenção RH), gráfico de barras de score por área, e um índice ("Neste relatório: Visão Geral, Área X, Área Y..., Pontos de Atenção para o RH").
2. **Uma página inteira por área** — cabeçalho compacto repetido (marca + nome da área + "Página X de N"), nome da área e score em destaque, e a lista de colaboradores dessa área com espaço de sobra (nada mais espremido em card pequeno).
3. **Uma página final de RH** (só aparece se houver algum sinalizado) — cabeçalho compacto + a faixa de alerta + os cards de cada pessoa sinalizada, com espaço de sobra igual às páginas de área.

Implementado com uma classe `.page` (`page-break-after: always`, removido no último elemento via `:last-of-type` e reforçado em `@media print`) — cada seção lógica (capa, cada área, RH) é o seu próprio bloco `.page`, então o Chromium sempre começa cada um numa página nova, sem depender de cálculo de altura ou de cortar conteúdo no meio.

**Itens curtos como "chips" (v1.8.2):** pontos fortes, pontos a desenvolver e motivos de valorização do RH agora são renderizados como etiquetas arredondadas lado a lado (`display:flex;flex-wrap:wrap`), não como lista de bullets — combina melhor com itens curtos (uma palavra/expressão) do que com frases, e deixa claro visualmente que são "tags", não parágrafos.

**Histórico da correção de layout — por que passou por 3 versões:** a v1.8.0 tentava caber tudo numa única página A4 paisagem, com fontes minúsculas (6-9px). Na v1.8.1, o grid de cards (`display:grid`) foi trocado por `display:flex;flex-wrap:wrap` + `break-inside:avoid`, porque o Chromium não fragmenta bem `display:grid` através de quebras de página impressas — resolvia cortes no meio de um card, mas o documento continuava apertado numa única página. O Erick então pediu pra abandonar de vez a restrição de uma página só e ganhar espaço de verdade — daí a reformulação em páginas dedicadas da v1.8.2 acima, que elimina o problema na raiz (cada seção tem a página inteira, não precisa mais competir por espaço vertical).

### 5.62 Análise IA passa a gerar também itens curtos (pontos fortes/melhoria/valorização) — v1.8.2

**Motivação:** o Relatório Executivo (seção 5.60) precisava de pontos fortes, pontos de melhoria e motivos de valorização como ITENS curtos (uma palavra ou expressão pequena), não frases. Tentar extrair isso de um texto livre já pronto (via corte de frase — v1.8.1) sempre esbarrava no mesmo problema: texto livre não nasceu formatado como item. A solução foi pedir os itens curtos direto pra IA, na mesma chamada que já gera a Análise Preditiva do relatório individual — ela já enxerga todos os comentários e o contexto da pessoa, então consegue sintetizar itens de verdade em vez de recortar frases.

**O que mudou (`AnthropicService.generateAnalysis`, `src/anthropic/anthropic.service.ts`):**
- O prompt enviado à IA (`buildPrompt`) ganhou 3 novos campos no JSON de resposta esperado: `strengthsItems`, `improvementItems`, `valuationItems` — cada um uma lista de **exatamente 3 strings curtas** (o prompt dá exemplos explícitos: "Disciplina", "Entregas consistentes", "Colaboração com o time" — e instrui a IA a nunca usar frase inteira nem truncar com "..."). `valuationItems` tem uma instrução própria: motivos pelos quais o RH deveria valorizar/reter a pessoa, sem falar de salário (o salário é tratado à parte, com dado real do sistema, não pela IA).
- Novo método privado `sanitizeItems()`: valida que a resposta da IA é um array de strings não vazias, tira ponto final acidental e corta em 3 itens (defesa contra a IA devolver mais ou menos do que pedido, ou algo malformado).
- `fallbackAnalysis()` (usado quando a API Key não está configurada ou a chamada falha) devolve os 3 arrays vazios — o Relatório Executivo já sabe lidar com array vazio (mostra "Sem análise IA gerada").

**Onde os itens são salvos (`PulseAiAnalysis`, `prisma/schema.prisma`):** 3 campos novos, `strengthsItems`/`improvementItems`/`valuationItems`, todos `String[] @default([])` (array nativo do Postgres) — adicionados SEM remover os campos de texto livre existentes (`strengths`, `improvements`, etc.), que continuam alimentando a seção "Análise Preditiva" do relatório individual exatamente como antes. `PulseReportsService.generateAiAnalysis()` (chamado quando o gestor clica "Gerar Análise IA" no relatório de alguém) grava os 3 arrays novos junto com os campos antigos, tanto no `create` quanto no `update` do upsert.

**Nada muda pro gestor no fluxo de uso:** o botão continua sendo "Gerar Análise IA", no mesmo lugar, com o mesmo comportamento — só que agora, por baixo, ele também popula os itens curtos usados no Relatório Executivo. Quando o gestor gera o Relatório Executivo depois, ele lê o que já foi salvo — sem custo nem espera extra de IA nesse momento, como decidido explicitamente com o Erick (alternativa descartada: gerar os itens na hora de exportar o PDF, que exigiria uma chamada de IA por colaborador a cada exportação).

### 5.63 Seletor de ciclo Pulse no Relatório Executivo — v1.8.3

**Motivação, levantada pelo Erick:** antes desta versão, o Relatório Executivo (seção 5.60) sempre pegava — por trás dos panos, sem avisar e sem deixar escolher — "o último ciclo finalizado/arquivado de cada área". Isso funciona bem enquanto só existe um ciclo fechado, mas o Erick notou o problema real: assim que um novo ciclo Pulse é aberto, não haveria como gerar o executivo referente ao ciclo ANTERIOR (o que acabou de fechar) sem esperar o próximo também fechar — o sistema sempre pularia pro mais recente disponível. A pergunta dele — "o sistema precisa me perguntar de qual pulse quero o relatório" — foi confirmada como a solução certa.

**Premissa confirmada com o Erick antes de implementar:** as áreas geridas por um gestor fecham o Pulse **juntas** (mesmo ciclo pra todas). Isso simplifica a solução — um único seletor por ciclo (não por área) resolve o caso de uso real. Se um dia isso deixar de ser verdade (uma área ficar num ritmo diferente das outras), o sistema já lida com isso sem quebrar: ao gerar o relatório, cada área é checada individualmente contra o ciclo escolhido, e uma área que não tiver esse ciclo específico fechado simplesmente fica de fora do relatório daquela vez (mesmo comportamento de antes pra "área sem ciclo consolidado").

**O que mudou:**
- **Novo endpoint** `GET /pulse-reports/one-page/cycles` (role GESTOR) — `OnePageExecutivaService.listCycles()` retorna a lista de ciclos FINALIZADO/ARQUIVADO que tocam qualquer uma das áreas geridas pelo gestor logado (mesmo `areaGroupWhere()` de sempre, agora aplicado a TODAS as áreas geridas de uma vez, não uma a uma), ordenados do mais recente pro mais antigo.
- **`OnePageExecutivaService.build()` ganhou um segundo parâmetro opcional, `cycleId`.** Quando informado, esse MESMO ciclo é usado pra todas as áreas (em vez de cada área buscar "seu" ciclo mais recente de forma independente, como era antes). Sem `cycleId` (chamada antiga/direta pela API), cai no fallback de sempre: o último ciclo fechado que toca qualquer área gerida — mantém compatibilidade com quem chamar o endpoint sem passar o parâmetro novo.
- **Endpoint do PDF** (`GET /pulse-reports/one-page/pdf`) ganhou o query param opcional `?cycleId=...`, repassado direto pro `build()`.
- **`cicloResumo`** (mostrado no topo da capa do PDF) deixou de ser o texto genérico "ciclo mais recente de cada área" e passou a mostrar o rótulo do ciclo específico escolhido (ex: "Ciclo: Pulse Setembro/2026") — mais transparente sobre qual Pulse aquele relatório retrata.

**Tela (`/relatorios`):** o botão "Gerar Relatório Executivo" agora abre um modal de seleção **sempre** (decisão explícita, pra manter o fluxo previsível mesmo quando só existe um ciclo fechado) — busca os ciclos disponíveis, marca o mais recente por padrão (rótulo "Mais recente"), e só chama o endpoint do PDF depois de o gestor confirmar (ou trocar) o ciclo e clicar em "Gerar PDF". Se nenhuma área gerida tiver ciclo fechado ainda, o modal avisa isso em vez de mostrar uma lista vazia.

> **Superado pela v1.8.5 (seção 5.65):** o seletor deixou de ser single-select — virou multi-seleção por checkbox, sem nenhum ciclo pré-marcado por padrão. `cicloResumo` (texto único citado acima) foi substituído por `ciclosUsados` (lista). O restante desta seção (endpoint `one-page/cycles`, premissa de que as áreas fecham juntas, fallback pro ciclo mais recente) continua válido.
