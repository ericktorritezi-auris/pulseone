...
**Segunda causa raiz, depois de corrigir a primeira:** `libnss3.so: cannot open shared object file` — mesmo o Chromium "enxuto" do `@sparticuz/chromium` ainda depende de bibliotecas de sistema (NSS) que o container Nixpacks do Railway não tem instaladas por padrão. **Correção definitiva:** em vez de depender de qualquer Chromium empacotado dentro do `node_modules`, o `nixpacks.toml` (novo, raiz do repo) instrui o Railway a instalar o **Chromium como pacote de sistema** via Nix (`nixPkgs = ["chromium"]`) — o Nix resolve a árvore de dependências completa e corretamente, sem faltar nenhuma biblioteca. `PulseReportPdfService.resolveChromiumPath()` localiza esse binário via `which chromium` em runtime (com fallbacks pra `chromium-browser`/`google-chrome-stable`) e aponta o `puppeteer-core` pra ele. `@sparticuz/chromium` foi removido — não é mais necessário.
