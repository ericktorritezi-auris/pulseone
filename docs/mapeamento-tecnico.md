...
**Quinta camada — timeout/diretório de perfil:** a causa real provável é o Chromium demorar mais que o timeout padrão do Puppeteer (30s) pra sinalizar prontidão num container com CPU limitada, ou ambiguidade de diretório de perfil de usuário. Corrigido: `timeout: 60_000` (dobra o prazo), `userDataDir` explícito e único por chamada (`fs.mkdtempSync`, com limpeza no `finally` — evita conflito entre PDFs gerados em paralelo), e `dumpio: true` pra ecoar a saída do Chromium em tempo real no log, garantindo diagnóstico completo se ainda houver falha na próxima tentativa.
