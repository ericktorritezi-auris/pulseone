...
### 5.60 One Page Executiva — resumo de uma página pro Gestor apresentar à Diretoria (v1.8.0, ajustado em v1.8.1)
...
- Pontos de força/melhoria: **(v1.8.1)** até **3 frases** de `PulseAiAnalysis.strengths`/`improvements` (a mesma análise por IA que já existe no relatório individual — seção da Análise Preditiva), extraídas via `topSentences()` — divide o texto em frases, corta cada uma no limite de caracteres sempre respeitando o fim de uma palavra (nunca no meio dela) e devolve as 3 primeiras, cada uma renderizada como item de uma lista de verdade (`<ul><li>`), em duas colunas ("Pontos fortes" / "A desenvolver"). **Correção de UX pedida pelo Erick**: na v1.8.0 era só a primeira frase, truncada de forma que às vezes cortava o texto visualmente no meio ("os destaques... acabam sendo cortados na tela. Isso não pode"); agora com 3 pontos completos por pessoa e truncamento seguro por palavra, isso não acontece mais. Se o gestor ainda não gerou a análise IA daquele relatório, aparece "Sem análise IA gerada para este colaborador neste ciclo" no lugar — não dispara geração automática (custaria uma chamada de IA por colaborador toda vez que alguém clicasse no botão).
...
**Correção de layout (v1.8.1):** o grid de cards de área (`.areas-grid`) e o grid de alertas de RH (`.rh-items`) usavam `display:grid`, que no Chromium não fragmenta bem através de quebras de página impressas — um card podia começar numa página e ser cortado no meio ao virar a página, em vez de simplesmente pular inteiro pra próxima. Trocado por `display:flex; flex-wrap:wrap` com `flex-basis` percentual nos filhos (efeito visual idêntico ao grid), combinado com `break-inside:avoid` em cada card/bloco de pessoa individualmente — agora, quando um bloco não cabe no resto da página, ele inteiro pula pra próxima em vez de ser cortado. Essa é a correção estrutural por trás do pedido do Erick ("os destaques... acabam sendo cortados na tela. Isso não pode").
